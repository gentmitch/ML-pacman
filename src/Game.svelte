<script>
  import { guess, playing } from "./stores.js";
  import { onMount } from "svelte";

  let canv = null;
  let pacman = {};
    
  canv = document.getElementById("pmcanvas");
...
pacman.engine = {
    ...
    render: function() {
        ...
        for (let y = 0; y < pacman.settings.h; y++) {
            for (let x = 0; x < pacman.settings.w; x++) {
...
<div>
  <canvas width="448" height="496" id="pmcanvas" />
  <div id="result-view" >
    <p on:click={() => pacman.engine.newGame('mitch')}>Replay</p>
  </div>
</div>
